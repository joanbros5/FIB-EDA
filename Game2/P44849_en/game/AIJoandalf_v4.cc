 ...
 */
#define PLAYER_NAME Joandalf_v4
...
struct PLAYER_NAME : public Player {
  ...
  static Player* factory () {
    return new PLAYER_NAME;
                                ...
                                //Joandalf_v4   -   copiado desde Joandalf_v3
                                //GANA 90% CONTRA DUMMYS, ligeramente mejor que Joandalf_v3
                                
                                //Changelog:    Variable global para controlar el radio de ir a por tesoros
                                //              Hecho bfs_conquer_general para que los dwarfs no hagan tonterías
                                //                 
                                //                      
                                //              
   ...
   */
    ...
    //BFS para ir hacia cuevas random, con diagonales
    Dir bfs_conquer_general(Pos actual, VVB Ocupat) {
        ...
        while (not Q.empty()) {
            ...
            //Casilla de cueva encontrada
            if (cell(P).owner != me() and cell(P).type != Outside and not Ocupat[P.i][P.j]) {
                return dirini;
            }
  ...
  virtual void play () {
      ...
      //Seteamos el radio para los tesoros
      radioTesoros = 200 - round();
    ...
    for (int i = 0; i < tamNegros; ++i) {
        ...
        //Prioridad: TODOS: Pegarse si tienen un mago o son atacados > ir a por tesoros si están cerca > conquer
        ...
        if (mago_cerca(negro.pos, 2) or enemigo_cerca_D(negro.pos, 1)) {
        ...
        else if (tesoro_cerca_D(negro.pos, min(25, radioTesoros), visitado)) {
            ...
            Dir palla = bfs_conquer_general(negro.pos, Ocupat);
            command(dniN, palla);
            Ocupat[(negro.pos + palla).i][(negro.pos + palla).j] = true;              
            ...
            if (!enemigo_cerca_D(negro.pos, 4)) {
                Dir palla = bfs_conquer_general(negro.pos, Ocupat);
                command(dniN, palla);
                Ocupat[(negro.pos + palla).i][(negro.pos + palla).j] = true;                
            }
