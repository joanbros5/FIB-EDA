 ...
 */
#define PLAYER_NAME Joandalf_v2
...
struct PLAYER_NAME : public Player {
  ...
  static Player* factory () {
    return new PLAYER_NAME;
                                ...
                                //Joandalf_v2   -   copiado desde Joandalf
                                //GANA 60% CONTRA DUMMIES, PIERDE CONTRA JOANDALF
                                
                                //Changelog:    BFS con accesos random
                                //              Todos - 2 a por tesoros
                                //              Dijkstra funciona (pero no se usa)
                                //              DFS tesoro con comparacion de granito y abyss
                                //              Conquers no comprueban que haya alguien en el bfs
   ...
   */
    ...
    bool tesoro_cerca_D(Pos actual, int x) {
        if (not pos_ok(actual) or cell(actual).type == Granite or cell(actual).type == Abyss) return false;
        if (x < 0) return false;
        else {
          Cell cuadrado = cell(actual);
          if (cuadrado.treasure) return true;
          return tesoro_cerca_D(actual + Bottom, x-1) or tesoro_cerca_D(actual + BR, x-1) or
          tesoro_cerca_D(actual + Right, x-1) or tesoro_cerca_D(actual + RT, x-1) or 
          tesoro_cerca_D(actual + Top, x-1) or tesoro_cerca_D(actual + TL, x-1) or 
          tesoro_cerca_D(actual + Left, x-1) or tesoro_cerca_D(actual + LB, x-1);
      }
    }
    ...
    Dir bfs_conquer_neutral(Pos actual, VVB Ocupat) {
        ...
        while (not Q.empty()) {
            ...
            if (cell(P).id == -1 and cell(P).type != Outside and not Ocupat[P.i][P.j]) {
                ...
                Visitado[P.i][P.j] = true;
                for (int count = 0; count < 8; ++count) {
                    Pos next = P + direc_D[random[count]];
                    if (pos_ok(next) and ((cell(next).type == Cave or cell(next).type == Outside) and (not Visitado[next.i][next.j]) and (not Ocupat[next.i][next.j]))) {
                        Q.push(make_pair(next, dirini));
                    }
                }
  ...
  virtual void play () {
    ...
    for (int i = 0; i < tamNegros; ++i) {
        ...
        //Prioridad: minConquer > Tesoros > conquer > conquer_E+pegarse
        
        if (i == 1 or i == 2) {
            if (enemigo_cerca_D(negro.pos, 1) or mago_cerca(negro.pos, 2)) {
        ...
        //Conquer
        else if (i > 1*(tamNegros/3) and i <= 2*(tamNegros/3)) {
            
            if (enemigo_cerca_D(negro.pos, 1)) {
                Dir palla = bfs_enemigo(negro.pos, Ocupat);
                command(dniN, palla);
                Ocupat[(negro.pos + palla).i][(negro.pos + palla).j] = true;
            }
        ...
        //Conquer casillas enemigas + pegarse si hay un mago con ellos
        else if (i > 2*(tamNegros/3) and i <= 3*(tamNegros/3)) {
            
            if (enemigo_cerca_D(negro.pos, 2) and mago_cerca(negro.pos, 2)) {
                Dir palla = bfs_enemigo(negro.pos, Ocupat);
                command(dniN, palla);
                Ocupat[(negro.pos + palla).i][(negro.pos + palla).j] = true;
            }
        ...
        //Si sobra alguien (multiplicidad % 3 por tanto pueden sobrar 1 o 2):
        //Kamikazes (solo van hacia enemigos si hay alguno cerca), else conquistar neutral
        else {
            
            if (enemigo_cerca_ND(negro.pos, 1)) {
                Dir palla = bfs_enemigo(negro.pos, Ocupat);
                command(dniN, palla);
                Ocupat[(negro.pos + palla).i][(negro.pos + palla).j] = true;
            }
