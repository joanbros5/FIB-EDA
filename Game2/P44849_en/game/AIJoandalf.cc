...
struct PLAYER_NAME : public Player {
  ...
  static Player* factory () {
    return new PLAYER_NAME;
                                ...
                                //Joandalf   -   Original, versión 1
                                //GANA 70% CONTRA DUMMIES, CÓDIGO BÁSICO, Se aprovecha de Dummys, Decente overall
   ...
   */
    ...
    //DFS para saber si hay un tesoro en un radio x (con diagonales)
    ...
    //BFS DIJKSTRA para llegar a un tesoro (con diagonales) // Cueva = 1 casilla, piedra = 5 casillas
    Dir bfs_tesoro(Pos actual, VVB Ocupat) {
            ...
            if (cell(P).treasure and not Ocupat[P.i][P.j]) {
                ...
                for (int count = 0; count < 8; ++count) {
                    ...
                    if (pos_ok(next) and ((cell(next).type == Cave or cell(next).type == Outside or cell(next).type == Rock) and (not Visitado[next.i][next.j]) and (not Ocupat[next.i][next.j]))) {
                        ...
                        else if (cell(next).type == Rock) Q.push(make_pair(dist + 1, make_pair(next, direc_D[count])));
                    }
                }
            }
        }
    ...
    */
    ...
    Dir bfs_huir(Pos actual, VVB Ocupat) {
        ...
        while (not Q.empty()) {
            ...
            //Enemigo encontrado
            if (cell(P).id != -1){
                if (unit(cell(P).id).player != me() and (cell(P).type != Rock and cell(P).type != Abyss and cell(P).type != Granite)) {
                    return opposite(dirini);
                }
            }
